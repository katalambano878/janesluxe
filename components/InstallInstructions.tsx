  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-[#AB9462]/10 text-[#AB9462] rounded-lg hover:bg-[#AB9462]/20 transition-colors whitespace-nowrap"
      >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[10000] p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 animate-scale-in">
            ...
            <div className="space-y-4 mb-6">
              {instructions.steps.map((step, index) => (
                <div key={index} className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-[#AB9462]/10 rounded-full flex items-center justify-center flex-shrink-0">
                    <i className={`${step.icon} text-[#AB9462]`}></i>
                  </div>
                  <div className="flex-1 pt-2">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="w-6 h-6 bg-[#2C1D00] text-white rounded-full flex items-center justify-center text-xs font-bold">
                        {index + 1}
                      </span>
            ...
            <button
              onClick={() => setShowModal(false)}
              className="w-full bg-[#2C1D00] hover:bg-[#2C1D00] text-white py-3 rounded-lg font-semibold transition-colors whitespace-nowrap"
            >
              Got It!
            </button>
          </div>

...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // 1-3. Create the order, items, and customer record via the server-side
      // checkout API. It uses the Supabase service role, so checkout works even
      // when the browser Supabase client isn't configured on the deployment
      // (avoids the "Failed to fetch" error from a placeholder Supabase URL).
      ...
      if (!checkoutRes.ok) {
        throw new Error(checkoutJson.error || 'Failed to create order');
      }
    ...
    return (
      <main className="min-h-screen bg-gray-50 py-20">
        <div className="max-w-md mx-auto text-center px-4">
          ...
          <Link href="/shop" className="inline-block bg-[#7A5C4D] text-white px-8 py-3 rounded-lg font-semibold hover:bg-[#7A5C4D]/90 transition-colors">
            Return to Shop
          </Link>
        </div>
      </main>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <button
                ...
                className={`p-6 rounded-xl border-2 transition-all text-left cursor-pointer ${checkoutType === 'guest'
                  ? 'border-[#7A5C4D] bg-[#F5EADF]'
                  : 'border-gray-200 hover:border-gray-300'
                  } ${user ? 'opacity-50 cursor-not-allowed' : ''}`}
                disabled={!!user}
              >
                <div className="flex items-center justify-between mb-3">
                  <i className="ri-user-line text-3xl text-[#7A5C4D]"></i>
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${checkoutType === 'guest' ? 'border-[#7A5C4D] bg-[#7A5C4D]' : 'border-gray-300'
                    }`}>
                ...
                {user && <p className="text-xs text-[#D7A7A0] mt-2">You are logged in</p>}
              </button>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <i className="ri-account-circle-line text-3xl text-[#7A5C4D]"></i>
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${checkoutType === 'account' ? 'border-[#7A5C4D] bg-[#7A5C4D]' : 'border-gray-300'
                    }`}>
              ...
              <>
                <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-[#D7A7A0] focus:border-[#D7A7A0] ${errors.firstName ? 'border-[#C4877B]' : 'border-gray-300'
                            }`}
                          placeholder="John"
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-[#D7A7A0] focus:border-[#D7A7A0] ${errors.lastName ? 'border-[#C4877B]' : 'border-gray-300'
                            }`}
                          placeholder="Doe"
                        />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-[#D7A7A0] focus:border-[#D7A7A0] ${errors.email ? 'border-[#C4877B]' : 'border-gray-300'
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-[#D7A7A0] focus:border-[#D7A7A0] ${errors.address ? 'border-[#C4877B]' : 'border-gray-300'
                          }`}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-[#D7A7A0] focus:border-[#D7A7A0] bg-white ${errors.region ? 'border-[#C4877B]' : 'border-gray-300'
                            }`}
                        >
                    ...
                    {checkoutType === 'account' && (
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input
                          ...
                          className="w-5 h-5 text-[#7A5C4D] rounded border-gray-300 focus:ring-[#D7A7A0]"
                        />
                  ...
                  <button
                    ...
                    className="w-full mt-6 bg-[#7A5C4D] hover:bg-[#7A5C4D]/90 text-white py-4 rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer"
                  >
            ...
            {currentStep === 2 && (
              <>
                <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                  ...
                  <div className="space-y-4">
                    <label className={`flex items-center justify-between p-4 border-2 rounded-lg cursor-pointer transition-colors ${deliveryMethod === 'pickup' ? 'border-[#7A5C4D] bg-[#F5EADF]' : 'border-gray-300 hover:border-gray-400'
                      }`}>
                      ...
                      <p className="font-bold text-[#7A5C4D]">FREE</p>
                    </label>

                    <label className={`flex items-center justify-between p-4 border-2 rounded-lg cursor-pointer transition-colors ${deliveryMethod === 'doorstep' ? 'border-[#7A5C4D] bg-[#F5EADF]' : 'border-gray-300 hover:border-gray-400'
                      }`}>
                      <div className="flex items-center space-x-4">
                        <input
                          type="radio"
                      ...
                      <p className="font-semibold text-[#D7A7A0] text-sm">At a Cost</p>
                    </label>

                    {/* Comprehensive delivery options - to be re-enabled later
                    <label className={`flex items-center justify-between p-4 border-2 rounded-lg cursor-pointer transition-colors ${deliveryMethod === 'accra' ? 'border-[#7A5C4D] bg-[#F5EADF]' : 'border-gray-300 hover:border-gray-400'
                      }`}>
                      <div className="flex items-center space-x-4">
                    ...
                    <label className={`flex items-center justify-between p-4 border-2 rounded-lg cursor-pointer transition-colors ${deliveryMethod === 'outside-accra' ? 'border-[#7A5C4D] bg-[#F5EADF]' : 'border-gray-300 hover:border-gray-400'
                      }`}>
                      <div className="flex items-center space-x-4">
                  ...
                  <div className="flex flex-col-reverse md:flex-row gap-4 mt-6">
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="flex-1 bg-[#7A5C4D] hover:bg-[#7A5C4D]/90 text-white py-4 rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer disabled:opacity-70 flex items-center justify-center"
                    >
                      {isLoading ? (
                        <>

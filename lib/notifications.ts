...
const EMAIL_FROM = process.env.EMAIL_FROM || 'YOUR_BRAND_NAME <noreply@yourdomain.com>';
const BRAND = {
    name: "JANE'S LUXE",
    color: '#D7A7A0',     /* Soft Dusty Rose */
    colorLight: '#F5EADF', /* Warm Nude Cream */
    colorDark: '#7A5C4D',  /* Warm Mocha */
    url: (process.env.NEXT_PUBLIC_APP_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null) || 'https://janesluxe.com').replace(/\/+$/, ''),
    phone: process.env.NEXT_PUBLIC_CONTACT_PHONE_DODOWA || '0598057757',
};
...
export async function sendOrderConfirmation(order: any) {
    ...
    const baseUrl = (process.env.NEXT_PUBLIC_APP_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null) || 'https://yourdomain.com').replace(/\/+$/, '');

    // Idempotency: skip if confirmation already claimed (callback + verify race).
    ...
    try {
        ...
        if (freshMeta.confirmation_sent_at) {
            console.log(`[Notification] Skipping duplicate confirmation for #${order_number || id}`);
            return { skipped: true };
        }
        await supabaseAdmin
        ...
        console.warn('[Notification] Could not claim confirmation lock:', claimErr?.message);
    }
...
`, `New order #${order_number} from ${name}`);
    ...
    if (phone) {
        const smsMessage = trackingNumber
            ...
            : `Hi ${name}, your order #${order_number || id} at YOUR_BRAND_NAME is confirmed! Track here: ${trackingUrl}${shippingNotesSms}`;
...
`, `Welcome to ${BRAND.name}, ${firstName}!`)
    ...
    if (phone) {
        await sendSMS({
            to: phone,
            message: `Welcome ${firstName}! Thanks for joining YOUR_BRAND_NAME.`
        });
    }
}
